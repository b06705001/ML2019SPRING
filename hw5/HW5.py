...
# Our 'cost' will be the likelihood out image is the target class according to the pre-trained model
    ...
    cost_function = model_output_layer[0, label[n]]
    if object_type_to_fake!=label[n]:
        cost_function_true =K.categorical_crossentropy(model_output_layer,K.one_hot(label[n],1000))
        cost_function_false=K.categorical_crossentropy(model_output_layer,K.one_hot(object_type_to_fake,1000))
    else:
        cost_function_true =K.categorical_crossentropy(model_output_layer,K.one_hot(label[n],1000))
        cost_function_false=K.categorical_crossentropy(model_output_layer,K.one_hot(label[n]+1,1000))
    max_change_above = array_of_img[n] + 5
    max_change_below = array_of_img[n] - 5
...
# In this case, referring to "model_input_layer" will give us back image we are hacking.
    ...
    grab_cost_and_gradients_from_model = K.function([model_input_layer, K.learning_phase()], [cost_function,-(cost_function_true)+(cost_function_false), gradient_function])
    ...
    # Keras layers behave differently in prediction vs. train modes!
        ...
        cost,loss, gradients = grab_cost_and_gradients_from_model([img, 0])
    # Move the hacked image one step further towards fooling the model
    #   if(cost>-0.1 and cost<0):
    #       gradients-=gradient_temp
        img -=  (np.sign(gradients))
        epoch+=1
    ...
    del img
    del max_change_above
    del max_change_below
    gc.collect()
    im.close()
    if n%10==0:
        K.clear_session()
...
#   python HW5.py "C:\Users\peter yang\Desktop\MLHW5\images" "C:\Users\peter yang\Desktop\MLHW5"